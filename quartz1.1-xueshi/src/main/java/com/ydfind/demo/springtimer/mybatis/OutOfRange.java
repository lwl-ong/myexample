...
public class OutOfRange {
    ...
    public void queryOutOfRange() throws IOException {
        logger.error("科目二执行开始，时间为：" + new SimpleDateFormat("yyyyMMdd HH:mm:ss:SSS").format(new Date()));
        ...
        logger.error("科目二执行结束，时间为：" + new SimpleDateFormat("yyyyMMdd HH:mm:ss:SSS").format(new Date()));
        ...
        //设置表头信息
        String sheetTitle = "科二超出电子围栏" + datenum.substring(0,8);
        String[] title = {"地区", "驾校","车牌号"};
